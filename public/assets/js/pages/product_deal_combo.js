function calculate_deal_price() {
  ...
  let checked_product = $(
    ".deal-box .deal-product-extra .checkbox-choose-variant:checked"
  );
  let extra_product_prices = checked_product
    .closest(".deal-product-extra")
    .find(".deal-product-price");
  ...
  $(".deal-box #deal-sale-off-price").text(
    format_price(origin_price - total_price, true)
  );
...
function add_hidden_checkbox_select(checkbox) {
  ...
  if ($(checkbox).is(":checked")) {
    $(checkbox)
      .closest(".product-choose-variant")
      .find(`input[type="hidden"][name="${name_checkbox}"]`)
      .remove();
  } else {
    $(checkbox)
      .closest(".product-choose-variant")
      .append(`<input type="hidden" name="${name_checkbox}" value="0">`);
  }
}
function init() {
  calculate_deal_price();
  $(".deal-box .deal-product-extra .checkbox-choose-variant").each(function (
    index,
    checkbox
  ) {
...
$(document).ready(function () {
  ...
  function get_current_variant(popup) {
    let color_selected_id = popup
      .find("#select-color .selected")
      .attr("data-id");
    ...
    let current_variant = variants.find(function (variant) {
      if (
        variant.color == color_selected_id &&
        variant.origin == origin_selected_id &&
        variant.aspect == aspect_selected_id &&
        variant.storage == storage_selected_id
      )
        return variant;
    });
  ...
  $("body").on("click", '.deal-box [data-toggle="popup"]', function (event) {
    ...
    popup
      .find(
        "#group-color, #group-storage, #group-aspect, #group-origin, #group-warranty"
      )
      .hide();
    ...
    loadAjax(
      "/ajax/get-phone-variants-by-deal",
      { deal_id: deal_id, phone_id: phone_id, location_id: location_id },
      {
        ...
        success: function (result) {
          ...
          current_variant_id =
            div_product_item.find('input[name*="product_id"]').val() ?? 0;
          current_service_product_id =
            div_product_item.find('input[name*="service_product_id"]').val() ??
            0;
          let current_variant = variants.find(function (variant) {
            ...
            $.each(colors, function (key, item) {
              select_color.append(`
                            <div class="color-item ${
                              current_variant.color == item.id ? "selected" : ""
                            }" data-id = '${item.id}'
          ...
          if (storages.length != 0) {
            $.each(storages, function (key, item) {
              select_storage.append(`
                            <option value="${
                              item.id
                            }" ${current_variant.storage == item.id ? "selected" : ""}>${item.value}</option>
                        `);
            });
          ...
          if (origins.length != 0) {
            $.each(origins, function (key, item) {
              select_origin.append(`
                            <option value="${
                              item.id
                            }" ${current_variant.origin == item.id ? "selected" : ""}>${item.value}</option>
                        `);
            });
          ...
          if (aspects.length != 0) {
            $.each(aspects, function (key, item) {
              select_aspect.append(`
                            <option value="${
                              item.id
                            }" ${current_variant.aspect == item.id ? "selected" : ""}>${item.value}</option>
                        `);
            });
          ...
          if (warranties.length != 0) {
            select_warranty.append(
              '<option data-price="0" value="0">Mặc định</option>'
            );
            $.each(warranties, function (key, item) {
              select_warranty.append(`
                            <option value="${
                              item.soft_id
                            }" data-price="${item.price}" ${current_service_product_id == item.soft_id ? "selected" : ""}>
                                ${item.name}
                            </option>
                        `);
            });
  ...
  $("body").on("click", "#popup-variant #btn-submit", function () {
    ...
    let warranty_selected_name = popup
      .find("#select-warranty option:selected")
      .text();
    let warranty_selected_price = Number(
      popup.find("#select-warranty option:selected").attr("data-price")
    );
    if (!warranty_selected_id) warranty_selected_id = 0;
    let div_product_item = $(".deal-box .deal-product-item")
      .find(`.deal-choose-variant[data-phone_id="${current_variant.phone_id}"]`)
      .closest(".deal-product-item");
    div_product_item
      .find(".product-current-variant")
      .text(current_variant.variant_text);
    div_product_item
      .find(".product-current-warranty")
      .text(warranty_selected_name);
    div_product_item
      .find(".deal-product-price")
      .attr("data-price", current_variant.price + warranty_selected_price);
    div_product_item
      .find(".deal-product-price .deal-price")
      .text(format_price(current_variant.price + warranty_selected_price));
    div_product_item
      .find('input[name*="product_id"]')
      .val(current_variant.soft_id);
    div_product_item
      .find('input[name*="service_product_id"]')
      .val(warranty_selected_id);
    calculate_deal_price();
  });
  ...
  $("form#form-buy-deal").submit(function (e) {
    if (
      $(this).find(".deal-product-extra .checkbox-choose-variant:checked")
        .length == 0
    ) {
      e.preventDefault();
  ...
  $("body").on(
    "change",
    ".product-content-box .price_location select#location, .product-store-list select#location-message",
    function () {
      ...
      try {
        if (deal_box.length) {
          ...
          let product_type = deal_box
            .find('input[name="main_product_type"]')
            .val();
          let div_deal_product = deal_box.find(".deal-product");
          loadAjax(
            `/ajax/get-deal-products`,
            {
            ...
            {
              beforeSend: function () {
                div_deal_product.html(
                  '<div id="loading_image" style="height:100px"></div>'
                );
              ...
              success: function (result) {
                if (result.success) {
                  ...
                  div_deal_product
                    .find("img.lazy")
                    .lazyload({ effect: "fadeIn", failure_limit: 10 });
                  responsiveDeal();
                  deal_box.show();
...
$("body").on(
  "click",
  ".deal-box .deal-product-extra .deal-product-image",
  function () {
...
$("body").on("click", ".deal-item", function () {
  ...
  $("html, body").animate({ scrollTop: div_deal_box.offset().top - 20 }, 300);
});
...
function responsiveDeal() {
  ...
  if (width > 510) {
    ...
    div_deal_product.owlCarousel({
      ...
      onInitialized: function (event) {
        div_deal_box
          .find(".owl-item:has(.deal-product-plus)")
          .addClass("deal-product-plus");
        div_deal_box.show();
      },
    });
  }
